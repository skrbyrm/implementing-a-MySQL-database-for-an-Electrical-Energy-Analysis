...
This table is used to summarize the consumption data by month for each facility. It includes information such as the district and facility name, consumption data (active, inductive, and capacitive power), 
consumption rate (inductive_ratio and capacitive_ratio), and whether the facility is penalized for overconsumption.This table is used to summarize the consumption data by month for each facility. 
It includes information such as the district and facility name, consumption data (active, inductive, and capacitive power), consumption rate (inductive_ratio and capacitive_ratio), and whether the facility is penalized for overconsumption.
```
...
This MySQL stored procedure called update_all that disables foreign key checks, calls four other stored procedures (data_by_months, data_by_weeks, data_by_dates, and data_by_hours), and then re-enables foreign key checks.
```
DELIMITER ;;
